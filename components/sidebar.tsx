...
export function Sidebar({ studioId, studioName }: { studioId: string; studioName?: string }) {
  ...
  return (
    <div
      className="sidebar-nav flex flex-col py-4 overflow-hidden flex-shrink-0"
      style={{
    ...
    >
      <Link href="/" className="px-[10px] mb-6 flex items-center gap-3">
        ...
        <span className="sidebar-label text-white text-sm font-semibold whitespace-nowrap font-[var(--font-heading)]">
          {studioName || 'Studio'}
        </span>
      </Link>
        ...
        {NAV_ITEMS.map(item => {
          ...
          return (
            <Link key={item.path} href={href}
              ...
              <span className="sidebar-label text-sm whitespace-nowrap">
                {item.label}
              </span>
            </Link>
          );
